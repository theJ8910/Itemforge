...
--Base Container
ITEM.InvType		= nil;						--This can be the name of the inventory type your containers use
...
function ITEM:OnStartTouch( eEntity, eActivator, touchItem )
	if touchItem then
		...
		if inv then touchItem:ToInventory( inv ) end
	end
end
...
If an inventory panel is currently visible then it closes it.
]]--
function ITEM:HideInventory()
	local pnlInv = self:GetInventoryPanel();
	if !pnlInv then return end
...
When the container receives it's inventory it needs to make a reference to it
]]--
...
When the container loses it's inventory it needs to clear the reference to it
]]--
...
Containers have the standard item options in addition to a "Check Contents" option.
]]--
