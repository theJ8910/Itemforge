...
function ITEM:CreateTimer( vID, fDelay, iReps, fnCallback, ... )
	if !vID			then return self:Error( "Couldn't create timer - timer ID not given! ID needs to be a string or number.\n" ) end
	if !fDelay		then return self:Error( "Couldn't create timer - delay not given.\n" ) end
	if !iReps		then return self:Error( "Couldn't create timer - number of times to repeat not given.\n" ) end
	if !fnCallback	then return self:Error( "Couldn't create timer - function not given.\n" )  end
	...
	self:AdjustTimer( vID, fDelay, iReps, fnCallback, ... );
	self:StartTimer( vID );
...
fnCallback should be the function that will be called when the timer ends. This function will be given this item as it's first argument.
Any other arguments given to StartTimer will be given to fnCallback whenever it runs.

This returns the unique ID (a string) if successful, or false if unsuccessful.
]]--
function ITEM:SimpleTimer( fDelay, fnCallback, ... )
	if !fDelay		then return self:Error( "Couldn't create simple timer - delay not given. \n" ) end
	if !fnCallback	then return self:Error( "Couldn't create simple timer - function not given. \n" ) end
	...
	local strID = "simple_"..i;
	if self:CreateTimer( strID, fDelay, 1, function(...) self:DestroyTimer( strID ); fnCallback(...) end, ...) then
		return strID;
	end
...
vID should be a unique string or int that is used to identify this timer.
	Example: "ThrowTimer", "ExplodeTimer", "TickTimer", etc
fDelay should be the time (in seconds) to wait before calling the given function.
...
iReps is an optional number of times this timer repeats before being removed.
...
fnCallback should be the function that will be called when the timer ends. This function will be given this item as it's first argument.
Any other arguments given to StartTimer or AdjustTimer will be given to fnCallback whenever it runs. So if you did:
	function ITEM:WatchAlarm( pl )
		pl:PrintMessage( HUD_PRINTTALK, "BEEP BEEP! ALARM GOING OFF!\n" );
	end
...
function ITEM:AdjustTimer( vID, fDelay, iReps, fnCallback, ... )
	if !vID			then return self:Error( "Couldn't adjust timer - timer ID not given! ID needs to be a string or number.\n" ) end
	if !fDelay		then return self:Error( "Couldn't adjust timer - delay not given.\n" ) end
	if !iReps		then return self:Error( "Couldn't adjust timer - number of times to repeat not given.\n" ) end
	if !fnCallback	then return self:Error( "Couldn't adjust timer - function not given.\n" ) end
	
	--Create timers collection if it hasn't been created already
	if !self.Timers then self.Timers = {} end
	
	local strGMTimerID = "if_item"..self:GetID().."_"..vID;
	
	--Create this timer if it hasn't been created yet
	if !self:HasTimer( vID ) then
		self.Timers[ vID ] = strGMTimerID;
	end
	timer.Adjust( strGMTimerID, fDelay, iReps, fnCallback, self, ... );
end
...
NOTE: Pausing a timer is not the same as stopping it; when a timer is unpaused, it will continue where it left off.
Example: If a 5 second timer is paused at 4 seconds, then 1 second after being unpaused it will finish.
...
NOTE: Unpausing a timer is not the same as starting it; when a timer is unpaused, it will continue where it was paused.
If the timer is restarted, then it starts the timer from the beginning again.
...
Permanently stops an active timer.
