...
--Rock-It Launcher
ITEM.InventoryType		= "inv_rockit";								--What kind of inventory do Rock-It Launchers use?
ITEM.AutoUnloadDelay	= 0.3;										--How much time should we wait between unloading items?
ITEM.EjectFrom			= Vector( 9.2274, 0.3052, 5.4279 );			--Where do we eject items from?
ITEM.Spindown			= nil;										--Records the time that a spin up / spin down will end.
...
function ITEM:OnInit()
	...
	local inv = IF.Inv:Create( self.InventoryType );
...
function ITEM:GetWireOutputs( eEntity )
	return Wire_CreateOutputs( eEntity, { "Items" } );
end
