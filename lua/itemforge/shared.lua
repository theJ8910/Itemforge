...
This file is the foundation of Itemforge.

Version is stored here, and the module loading is carried out here.
Modules are loaded, then initialized. After that, client and server initilizations are performed respectively.
]]--
IF		   = {};
IF.Version = 0.26;								--Itemforge Version. Your items can use this if you want.
...
Clean up Itemforge.
This will call every module's cleanup functions and set Itemforge's table to nil to garbage collect it.
]]--
function IF:Cleanup()
	for k, v in pairs( self.Modules ) do
		if v.Cleanup then
	...
	--Remove serverside hooks
	if SERVER then
		hook.Remove( "PlayerInitialSpawn", "itemforge_send_item_inv" );
		hook.Remove( "PlayerDisconnected", "itemforge_cleanup_invs" );
	end
